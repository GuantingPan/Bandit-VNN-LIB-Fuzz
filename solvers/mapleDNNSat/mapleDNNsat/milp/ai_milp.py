...
def handle_conv(model, var_list, start_counter, filters, biases, filter_size, input_shape, strides, out_shape, pad_top,
    ...
    num_in_neurons = np.prod(input_shape)
    ...
    for j in range(num_out_neurons):
        ...
        var = model.addVar(vtype=GRB.CONTINUOUS,
                           lb=lbi[j], ub=ubi[j], name=var_name)
        var_list.append(var)
        ...
        for out_z in range(out_shape[1]):
            for out_x in range(out_shape[2]):
                for out_y in range(out_shape[3]):
                    ...
                    dst_ind = out_z * \
                        out_shape[2]*out_shape[3] + out_x*out_shape[3] + out_y
                    expr = LinExpr()
                    ...
                    for inp_z in range(input_shape[0]):
                        for x_shift in range(filter_size[0]):
                            for y_shift in range(filter_size[1]):
                                ...
                                expr.addTerms(
                                    filters[out_z][inp_z][x_shift][y_shift], var_list[src_ind])
        ...
        for out_x in range(out_shape[1]):
            for out_y in range(out_shape[2]):
                for out_z in range(out_shape[3]):
                    dst_ind = out_x * \
                        out_shape[2]*out_shape[3] + out_y*out_shape[3] + out_z
                    expr = LinExpr()
                    expr += -1*var_list[start+dst_ind]
                        ...
                        for x_shift in range(filter_size[0]):
                            for y_shift in range(filter_size[1]):
                             ...
                             # expr.addTerms(filters[filter_index],var_list[src_ind])
                                expr.addTerms(
                                    filters[x_shift][y_shift][inp_z][out_z], var_list[src_ind])
...
def handle_maxpool(model, var_list, layerno, src_counter, pool_size, input_shape, strides, output_shape, pad_top, pad_left, lbi, ubi, lbi_prev, ubi_prev, use_milp):
    ...
    start = len(var_list)
    # input_shape[0]*input_shape[1]*input_shape[2]
    num_neurons = np.prod(input_shape)
    ...
    for out_pos in range(output_size):
        ...
        if use_milp == 1:
            ...
            for l in range(len(pool_map)):
                ...
                expr = var_list[dst_index] - \
                    var_list[src_var] + cst*var_list[binary_var]
                model.addConstr(expr, GRB.LESS_EQUAL, cst)

                # indicator constraints
                model.addGenConstrIndicator(
                    var_list[binary_var], True, var_list[dst_index]-var_list[src_var], GRB.EQUAL, 0.0)
...
def handle_relu(model, var_list, affine_counter, num_neurons, lbi, ubi, relu_groupsi, use_milp, partial_milp_neurons):
    ...
    milp_encode_idx = cross_over_idx[:partial_milp_neurons] if partial_milp_neurons >= 0 else cross_over_idx
    ...
    for j in range(num_neurons):
        ...
        var = model.addVar(vtype=GRB.CONTINUOUS, lb=0.0,
                           ub=upper_bound,  name=var_name)
        var_list.append(var)
    ...
    if len(milp_encode_idx) > 0:
        for i, j in enumerate(milp_encode_idx):
            ...
            if(ubi[j] <= 0):
                ...
                # y <= x - l(1-a)
                expr = var_list[relu_counter+j] - \
                    var_list[affine_counter+j] - lbi[j] * var_bin
                ...
                model.addGenConstrIndicator(
                    var_bin, True, var_list[affine_counter+j], GRB.GREATER_EQUAL, 0.0)
    ...
    if len(relu_groupsi) > 0:
        _add_kactivation_constraints(
            model, var_list, relu_groupsi, affine_counter, relu_counter)
    else:
        for j in relax_encode_idx:
...
def handle_sign(model, var_list, affine_counter, num_neurons, lbi, ubi):
    ...
    for j in range(num_neurons):
        if(ubi[j] <= 0):
            ...
            # x >= l(1-a)
            expr = - var_list[affine_counter+j] - \
                lbi[j]*var_list[binary_counter+j]
            model.addConstr(expr, GRB.LESS_EQUAL, -lbi[j])

            # x <= u.a
            expr = var_list[affine_counter+j] - \
                ubi[j]*var_list[binary_counter+j]
            model.addConstr(expr, GRB.LESS_EQUAL, 0)

            # y = a
            expr = var_list[sign_counter+j]
            model.addConstr(expr, GRB.GREATER_EQUAL,
                            var_list[binary_counter+j])

            # indicator constraint
            model.addGenConstrIndicator(
                var_list[binary_counter+j], True, var_list[affine_counter+j], GRB.GREATER_EQUAL, 0.0)
...
def handle_tanh_sigmoid(model, var_list, affine_counter, num_neurons, lbi, ubi,
    ...
    for j in range(num_neurons):
        ...
        if activation_type == "Tanh":
            var = model.addVar(vtype=GRB.CONTINUOUS, lb=math.tanh(
                x_lb), ub=math.tanh(x_ub), name=var_name)
        else:
            var = model.addVar(vtype=GRB.CONTINUOUS, lb=sigmoid(
                x_lb), ub=sigmoid(x_ub), name=var_name)
        var_list.append(var)

    _add_kactivation_constraints(
        model, var_list, constraint_groups, affine_counter, y_counter)
...
def create_model(nn, LB_N0, UB_N0, nlb, nub, relu_groups, numlayer, use_milp, is_nchw=False, partial_milp=0, max_milp_neurons=-1):
    ...
    milp_activation_layers = np.nonzero(
        [l in ["ReLU", "Maxpool"] for l in nn.layertypes])[0]

    # Determine whcich layers, if any to encode with MILP
    if partial_milp < 0:
        partial_milp = len(milp_activation_layers)
    first_milp_layer = len(nn.layertypes) if partial_milp == 0 else milp_activation_layers[-min(
        partial_milp, len(milp_activation_layers))]
    ...
    if len(UB_N0) == 0:
        ...
        for i in range(num_pixels):
            ...
            var_name = "x" + str(i)
            var = model.addVar(vtype=GRB.CONTINUOUS,
                               lb=lower_bound, ub=upper_bound, name=var_name)
            var_list.append(var)
            expr = LinExpr()
        ...
        for i in range(num_pixels):
            var_name = "x" + str(i)
            var = model.addVar(vtype=GRB.CONTINUOUS,
                               lb=LB_N0[i], ub=UB_N0[i], name=var_name)
            var_list.append(var)
    ...
    for i in range(numlayer):
        if nn.layertypes[i] in ['SkipCat']:
            ...
            counter = handle_affine(
                model, var_list, counter, weights, biases, nlb[i], nub[i])
            ffn_counter += 1
            start_counter.append(counter)
            ...
            partial_milp_neurons = (
                first_milp_layer <= i) * (max_milp_neurons if max_milp_neurons >= 0 else len(nlb[i]))
            if relu_groups is None:
                counter = handle_relu(model, var_list, counter, len(
                    nlb[i]), nlb[index-1], nub[index-1], [], use_milp, partial_milp_neurons)
            else:
                counter = handle_relu(model, var_list, counter, len(
                    nlb[i]), nlb[index-1], nub[index-1], relu_groups[activation_counter], use_milp, partial_milp_neurons)
            ...
            index = nn.predecessors[i+1][0]
            counter = handle_sign(model, var_list, counter, len(
                nlb[i]), nlb[index-1], nub[index-1])
            activation_counter += 1
            start_counter.append(counter)
            ...
            index = nn.predecessors[i+1][0]
            counter = start_counter[index]
            counter = handle_maxpool(model, var_list, i, counter, pool_size, input_shape, strides,
                                     out_shape, padding[0], padding[1], nlb[i], nub[i], nlb[i-1], nub[i-1], use_milp)
            start_counter.append(counter)
            pool_counter += 1
            ...
            index = nn.predecessors[i+1][0]
            counter = start_counter[index]
            counter = handle_padding(model, var_list, counter, input_shape, out_shape,
                                     padding[0], padding[1], padding[2], padding[3], nlb[i], nub[i], is_nchw=is_nchw)
            start_counter.append(counter)
            pad_counter += 1
            ...
            counter = handle_residual(
                model, var_list, counter1, counter2, nlb[i], nub[i])
            start_counter.append(counter)
            residual_counter += 1
            ...
            assert 0, 'layertype:' + \
                nn.layertypes[i] + 'not supported for refine'
...
def get_bounds_for_layer_with_milp(nn, LB_N0, UB_N0, layerno, abs_layer_count, output_size, nlb, nub, relu_groups, use_milp, candidate_vars, timeout):
    ...
    counter, var_list, model = create_model(
        nn, LB_N0, UB_N0, nlb, nub, relu_groups, layerno+1, use_milp, partial_milp=-1, max_milp_neurons=-1)
    resl = [0]*len(lbi)
    resu = [0]*len(ubi)
    indices = []
...
def verify_network_with_milp(nn, LB_N0, UB_N0, nlb, nub, constraints, spatial_constraints=None, is_nchw=False):
    ...
    for or_list in constraints:
        ...
        for is_greater_tuple in or_list:
            obj = obj_from_is_greater_tuple_old(
                is_greater_tuple, var_list, counter)
            model.setObjective(obj, GRB.MINIMIZE)
            ...
            for cutoff in [0.01, 0.1, GRB.INFINITY]:
                model.reset()
                milp_timeout = config.timeout_final_milp if config.timeout_complete is None else (
                    config.timeout_complete + start_milp - time.time())
                model.setParam(GRB.Param.TimeLimit, milp_timeout)
                model.setParam(GRB.Param.Cutoff, cutoff)
                model.optimize(milp_callback)
                # status 3 and 4 indicate an infeasible model
                if model.status not in [3, 4]:
                    # no infeasibility reported.
                    break
                else:
                    warnings.warn(
                        "Infeasible model encountered. Trying to increase the Cutoff parameter to recover.")
            else:
                # all values led to an infeasible model
                assert model.status not in [
                    3, 4], f"Infeasible model encountered. Model status {model.status}"

            obj_bound = f"{model.objbound:.4f}" if hasattr(
                model, "objbound") else "failed"
            obj_val = f"{model.objval:.4f}" if hasattr(
                model, "objval") else "failed"
            print(
                f"MILP model status: {model.Status}, Obj val/bound for constraint {is_greater_tuple}: {obj_val}/{obj_bound}, Final solve time: {model.Runtime:.3f}")
        ...
        if not or_result:
            ...
            if len(or_list) > 1:
                ...
                for i, is_greater_tuple in enumerate(or_list):
                    obj_constr = obj_from_is_greater_tuple_old(
                        is_greater_tuple, var_list, counter)
                    model.addConstr(obj_constr, GRB.LESS_EQUAL,
                                    0, name=f"Adex_Obj_{i:d}")
                milp_timeout = config.timeout_final_milp if config.timeout_complete is None else (
                    config.timeout_complete + start_milp - time.time())
                model.setParam(GRB.Param.TimeLimit, milp_timeout)
                ...
                model.reset()
                model.optimize()
                sol_count = f"{model.solcount:d}" if hasattr(
                    model, "solcount") else "None"
                print(
                    f"MILP adex model status: {model.Status}, Model solution count: {sol_count}, Final solve time: {model.Runtime:.3f}")
                if model.solcount > 0:
                ...
                # model is infeasible
                if certify_with_feasibility and model.status in [3, 4]:
                    ...
                    if model.status not in [3, 4]:  # not infeasible anymore
                        print(
                            f"MILP adex model status without adex constraints: {model.Status}, Final solve time: {model.Runtime:.3f}")
                        warnings.warn(
                            "Model feasibility used for certification.")
                        or_result = True
        if not or_result:
