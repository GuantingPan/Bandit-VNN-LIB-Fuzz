    ...
    def match(
        ...
        for layer_type in layer_types:
            if layer_type.OP_PATTERN is None:
                continue
            matches = layer_type.OP_PATTERN.match(
                operation_graph.output_operations)
        ...
        op_graph = OperationGraph(
            operation_graph.walk(DropPrefix(input_op_graph)))
        return LayerMatch(
...
class Convolutional(Layer):
    ...
    def from_operation_graph(cls, operation_graph):
        ...
        if isinstance(op, Conv):
            ...
            raise ValueError("Expected type Conv, but got %s" %
                             op.__class__.__name__)
        return cls(
            weights,
            bias,
