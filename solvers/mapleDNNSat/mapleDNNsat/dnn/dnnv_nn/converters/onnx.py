...
class OnnxConverter(OperationVisitor):
    ...
    def convert(self, name="onnx_model") -> onnx.ModelProto:
        ...
        nodes = [n for n in self.visited.values() if isinstance(n,
                                                                onnx.NodeProto)]
    ...
    def _to_onnx_proto(
        ...
        if isinstance(value, Operation):
            ...
            return tensor_proto
        raise ValueError(
            f"Unknown type for operand of {opname}: {type(value)}")
    ...
    def visit_BatchNormalization(
        ...
        variance = self._to_onnx_proto(
            operation.variance, f"{opname}.variance")
