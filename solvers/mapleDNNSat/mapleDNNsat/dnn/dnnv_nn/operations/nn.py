    ...
    def __init__(
        ...
        *,
        ...
        self.dilations = np.asarray(
            dilations if dilations is not None else (
    ...
    def __init__(
        ...
        *,
        ...
        if self.output_shape is None:
            self.pads = np.asarray(
                pads if pads is not None else (
                    0,) * (len(self.kernel_shape) * 2)
            )
        else:
...
class MaxPool(Operation):
    ...
    def from_onnx(cls, onnx_node, *inputs):
        ...
        kernel_shape = attributes.get("kernel_shape")
        pads = attributes.get("pads", 0)
        storage_order = attributes.get(
            "storage_order", MaxPool.ROW_MAJOR_STORAGE)
        strides = attributes.get("strides", 1)
        return cls(
            *inputs,
            kernel_shape,
