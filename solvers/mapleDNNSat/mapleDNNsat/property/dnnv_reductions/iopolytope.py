...
from ...dnn.dnnv_nn import OperationGraph, OperationTransformer
from ..dnnv_properties import (
    Add,
    And,
    Constant,
    Expression,
...
Halfspace = namedtuple(
    "Halfspace", ["indices", "coefficients", "b", "is_open"])
    ...
    def _update_bounds(self, indices, coefficients, b, is_open=False):
        ...
        if len(indices) == 1:
            ...
            for i in indices:
                ...
                try:
                    obj[i] = 1
                    result = linprog(obj, A_ub=self.A,
                                     b_ub=self.b, bounds=(None, None))
                    if result.status == 0:
...
class HyperRectangle(HalfspacePolytope):
    ...
    def add_variable(self, variable):
        ...
        size = variable.size()
        self._lower_bound = np.concatenate(
            [self._lower_bound, np.zeros(size) - np.inf])
        self._upper_bound = np.concatenate(
            [self._upper_bound, np.zeros(size) + np.inf])
        return self
    ...
    def update_constraint(self, variables, indices, coefficients, b, is_open=False):
        ...
        if coef < 0:
            if is_open:
                value = np.nextafter(value, value + 1)
            self._lower_bound[flat_index] = max(
                value, self._lower_bound[flat_index])
        elif coef > 0:
            if is_open:
                value = np.nextafter(value, value - 1)
            self._upper_bound[flat_index] = min(
                value, self._upper_bound[flat_index])
...
class IOPolytopeProperty(Property):
        ...
        class Merger(OperationTransformer):
            ...
            def visit_Input(self, operation):
                ...
                if input_details not in self.input_operations:
                    self.input_operations[input_details] = self.generic_visit(
                        operation)
    ...
    def prefixed_and_suffixed_op_graph(
        ...
        class PrefixTransformer(OperationTransformer):
            ...
            def visit_Input(
                ...
                if len(input_shape) == 2:
                    ranges = self.ubs[self._input_count] - \
                        self.lbs[self._input_count]
        ...
        prefixed_op_graph = OperationGraph(
            suffixed_op_graph.walk(prefix_transformer))
        return (
    ...
    def suffixed_op_graph(self) -> OperationGraph:
        # import ..dnn.dnnv_nn.operations as operations
    ...
    def visit_Network(self, expression: Network):
        ...
        if expression not in self.networks:
            ...
            if self.output_constraint is None:
                ...
                self.output_constraint = self.output_constraint.add_variable(
                    variable)
        variable = Variable(
            self._network_output_shapes[expression], str(expression))
    ...
    def visit_Symbol(self, expression: Symbol):
        if self.input is None:
            ...
            if expression not in self._network_input_shapes:
                raise self.reduction_error(
                    f"Unknown shape for variable {expression}")
            variable = Variable(
